...
app.MapPut("/tarefas/alterar/{id}", ([FromServices] AppDataContext ctx, [FromRoute] int id, [FromBody] bool statusConcluido) =>
{
    ...
    Tarefa.Concluida = statusConcluido;
    ctx.SaveChanges();
...
{
    var tarefasNaoConcluidas = ctx.Tarefas.Where(t => !t.Concluido).ToList();
    if (tarefasNaoConcluidas.Any())
    {
        return Results.Ok(tarefasNaoConcluidas);
    }
    return Results.NotFound("Nenhuma tarefa nao concluida encontrada!");
...
{
    var tarefasConcluidas = ctx.Tarefas.Where(t => t.Concluido).ToList();
